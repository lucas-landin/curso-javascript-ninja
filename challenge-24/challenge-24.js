...
function isLastItemAnOperation(strig) {
  var operations = getOperations();
  var lastItem = strig.split('').pop();
...
function getOperations(){
  Array.prototype.map.call($buttonsOperations,function(button){
    return button.value
  })
}

function removeLastItemIfItIsAnOperator(strig) {
  if(isLastItemAnOperation(strig)) {
    return strig.slice(0, -1);
  }
  return strig;
}
